...
def encode_chromosome2(model, n_appr_levels = 255, axx_linear=False):
    """
    Return the number of Conv layers in the model, that is the core info to encode chromosome
    """
    n_layers = 0
        ...
        if isinstance(module, nn.Conv2d) or isinstance(module, AdaptConv2D):
            ...
            for sub_name, sub_module in module._modules.items():
                if isinstance(sub_module, ResidualModule):
                    for subsub_name, subsub_module in sub_module._modules.items():
                        # if isinstance(subsub_module, nn.Conv2d) or isinstance(subsub_module, approxNN.AdaPT_Conv2d) or isinstance(module, nemo.quant.pact.PACT_Conv2d):
                        if isinstance(subsub_module, nn.Conv2d) or isinstance(subsub_module, AdaptConv2D):
                            n_layers += 1
        if axx_linear:
...
def list_mult_per_layer(model, imsize=(1, 3, 32, 32), axx_linear=False, device="cpu"):
    """
    ...
    model.eval()
    with torch.no_grad():
        in_t = torch.randint(low=0, high=256, size=imsize, dtype=torch.float32, device=device)
        print(f"in_t.device = {in_t.device}")
        print(f"in_t.shape = {in_t.shape}")
        _ = model(in_t)
...
def list_mult_per_layer2(model, imsize=(1, 3, 32, 32), axx_linear=False, device="cpu"):
    """
    @param model: torch.nn.Module
    @param imsize: tuple
    @param axx_linear: bool
    ...
    # Register hooks for all relevant layers
    for name, module in model.named_modules():
        if isinstance(module, (nn.Conv2d, AdaptConv2D)):
            module.register_forward_hook(get_activation(name))
        elif axx_linear and isinstance(module, (nn.Linear, AdaPT_Linear)):
            module.register_forward_hook(get_activation(name))
